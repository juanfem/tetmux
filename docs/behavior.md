...
Split out of `CLAUDE.md`. One entry per feature or mechanism, each recording what the behaviour is,
why it is that way, and which parts are load-bearing.
...
- **⌥ on a close or kill button skips the confirmation, and is read at click time.** The confirmation
  ...
  `ModifierKeyMonitor` and `MenuModifierMonitor` keep a *display* current and are allowed to be a frame
  ...
  run loop of its own), which is why the menu bar polls instead. Both report ⌥ **and** ⌘, since F4.36's
  copy is modified by the second one and appears on a window control and inside a menu; the sidebar
  therefore holds one of each, which is not duplication but two surfaces of one tree.
...
- **The copyable attach command is the one line here that is not an invocation (F4.36).**
  ...
  **⌘ held on either control asks the same question from a shell already on the host**
  ...
  from `CommandKey.isHeld` at click time, which is the division ⌥ already makes.
...
- **The menu bar extra says what ⌥ would do, by polling.** `MenuBarExtra` hands its content no event
  ...
  `MenuModifierMonitor` therefore reads the hardware flags on a `Timer` added to the **common** run-loop
