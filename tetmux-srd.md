...
- **F4.36** Any session can be copied as the **command line that reaches it from a shell** —
  ...
  *not* any invocation this application makes: those all spawn `tmux -CC`, and a person pasting one
  ...
  line the click will copy before it is clicked, since a clipboard is not somewhere a result can be
  checked, and neither offers the modifier on the local host, where it would change nothing.
